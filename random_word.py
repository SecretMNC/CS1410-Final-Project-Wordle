"""Obtains a random word from word_list.txt and returns it.
Programmatically fixes the text file if a formatting error is found.
Run this module to test random word getter."""
...
class WordPicker:
    ...
    def read_word_list(self):
        # Reads from text file, stores info in a dict, handles errors
        try:
    ...
    def prompt_fix_file(self):
        ...
        choice = input(
            "Would you like to update the word text file with "
            "only valid words?\n(1) for Yes\n(2) for No\n--> "
        )
        return choice == "1"
